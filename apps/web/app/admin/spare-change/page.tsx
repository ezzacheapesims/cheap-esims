...
export default function AdminSpareChangePage() {
  const { user } = useUser();
  ...
  const fetchBalance = async (userId: string) => {
    ...
    try {
      const data = await safeFetch<{ balanceCents: number }>(
        `${apiUrl}/admin/spare-change/${userId}`,
        {
          headers: {
            "x-admin-email": user?.primaryEmailAddress?.emailAddress || "",
          },
  ...
  const handleCredit = async () => {
    ...
    try {
      const result = await safeFetch<{ success: boolean; newBalance: number }>(
        `${apiUrl}/admin/spare-change/credit`,
        {
          method: "POST",
          headers: {
      ...
      toast({
        title: "Success",
        description: `Spare Change added successfully. New balance: $${(result.newBalance / 100).toFixed(2)}`,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to add Spare Change",
        variant: "destructive",
      });
    } finally {
      setCrediting(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black text-black uppercase tracking-tighter mb-2">Spare Change Management</h1>
        <p className="text-gray-600 font-mono font-bold uppercase text-sm">
          Add Spare Change to any user account
        </p>
      </div>
          ...
          <CardContent className="p-6">
            <div className="space-y-3">
              ...
              <div className="pt-4 mt-4 border-t-2 border-dashed border-black">
                <Label className="text-gray-500 font-mono font-bold text-xs uppercase">Spare Change Balance</Label>
                {loadingBalance ? (
        ...
        <Card className="bg-white border-2 border-black rounded-none shadow-hard overflow-hidden">
          <CardContent className="py-12 text-center border-dashed border-2 border-gray-200 m-4">
            <p className="text-gray-500 font-mono uppercase font-bold">
              Search for a user by email to view their Spare Change balance and add credits
            </p>
          </CardContent>
        </Card>
      )}
